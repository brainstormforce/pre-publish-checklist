...
if (!class_exists('BSFPPC_Loader')):
		...
		public function bsfppc_add_item() {
			...
			if (!empty($_POST['item_content'])) {
				...
				if (!empty($bsfppc_checklist_item_data)) {
					$bsfppc_item_exists_key = array_search($bsfppc_newitems, $bsfppc_checklist_item_data);
				}

				if (empty($bsfppc_item_exists_key)) {
	...
	$bsfppc_checklist_item_data = get_option('bsfppc_checklist_data');
				if (!empty($bsfppc_checklist_item_data)) {
					if (!empty($bsfppc_item_exists_key)) {
						?>
						<p class="warning bsfppc-alreadyexists-waring-description">List item already exists</p><?php
						}?>
						<?php foreach ($bsfppc_checklist_item_data as $key) {?>
